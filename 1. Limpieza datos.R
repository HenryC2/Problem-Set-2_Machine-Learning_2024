...
#-------------------------------------------------------------------------------
   ...
   ### Bases de hogares
      ...
      #Base de test
      names(test_hogares)  # No tiene variable de ingresos  ni relacionada a pobreza
      str(test_hogares)  
  ...
  ### Bases de personas
      ...
      #Base de test
      names(test_personas)  # No tiene variable de ingresos  ni relacionada a pobreza
      str(test_personas)    
      
#-------------------------------------------------------------------------------
# 3. Verificar que la variables de pobreza  -----------------------------------
#-------------------------------------------------------------------------------
...
#------------------------------------------------------------------------------
# 5. Tramiento de la base de personas ----------------------------------------
#------------------------------------------------------------------------------
      
#5.1  Dejar las variables que comparta la base de train y test ---------------
  train_personas <- train_personas[,c(colnames(test_personas))]
  
#5.2 . Missings variables de interes antes de agrupar por hogar --------------
  ...
  data_table_missing <- train_personas %>% 
    dplyr::select(P6020, #Sexo (Dummy)
                  ...
                  P6870, #Número de personas que tien empresa, negocio (categorica)
  ...
  #5.3. Crear las potenciales variables de interes ----------------------------
    ...
    #Funcion parar crear las variables al mismo tiempo en la base de entrenamiento y de test
      ...
      pre_process_personas <- function(data,...){
        ...
        #Variables de mercado laboral
            ...
            Des = ifelse(is.na(Des),0,Des), #Desocupados
            Ina = ifelse(is.na(Ina),0,Ina), #Desocupados
  ...
  #5.4 Agregar base a nivel de hogar -----------------------------------------
  ...
  #5.5. Crear base de variables a nivel jefe de hogar ------------------------
...
#------------------------------------------------------------------------------#
# 6.                     PROCESAR BASES DE HOGARES
#------------------------------------------------------------------------------#
    
          
  #6.1. Dejar las variables que se encuentas en amabs bases---------------------
  ...
  #6.2. Renombras y crear nuevas variables -------------------------------------
    ...
    pre_limp_hogares <- function(data,...){
      data <- data %>% 
        rename(n_cuartos = P5000,             #Cuartos en total disponible en el hogar
               ...
               pagm_armort = P5100,           #Pago mensual por cuota de amortización
        ...
        mutate(per_dor = Nper/n_cuartos_duermen, #Personas por cuarto
               hacinamiento =  ifelse(per_dor > 3, 1, 0)) #Si duermen mas de tres personas en el mismo cuarto está en hacinamiento
    }
...
#------------------------------------------------------------------------------#
  ...
  #7.1. Pegar bases de personas a nivel de hogar y solo informacion de jefe de hogar
...
#------------------------------------------------------------------------------#
#  8.             PEGAR BASES DE PERSONAS A NIVEL HOGAR 
#------------------------------------------------------------------------------#     
     
#8.1. Pegar las bases creadas de nivel persona-hogar con la base de hogar
     ...
     pre_process_hogares <- function(data,...){
      data <- data %>% 
        ...
        mutate(
          ...
          P5140 = ifelse(is.na(pagm_arriendo),pagm_arriendo_est,pagm_arriendo), #si no paga arriendo pone el valor que pagaria
          Ln_Cuota = log(pagm_armort),#Log de pago de cuota
          Ln_Cuota = ifelse(is.na(Ln_Cuota),0,Ln_Cuota), #pone 0 en NA
          P5100 = ifelse(is.na(pagm_armort),0,pagm_armort), #pone 0 en NA (valor cuota)
          #Ln_Est_arrien = log(P5130), #Log de pago de estimativo de pago de arriendo
          Ln_Pago_arrien = log(pagm_arriendo)) #Log de pago arriendo 
    }
...
#8.2. Cambios en variables de predicción 
...
#8.3. Mirar la variable de pobre despues de armado la muestra
